...
async function setupIndexes() {
  try {
    ...
    await db
      ...
      .createIndex({ channelId: 1, timestamp: -1 });
        ...
        if (channelsMap.has(channel.id)) {
          try {
            ...
            if (fetchedChannel) {
              ...
              console.log(
                `Channel ${fetchedChannel.name} has been automatically deleted.`
              );

              // Save the channel deletion event to the database
              withDb(async (db) => {
                const channelDeletionsCollection =
                  db.collection("channelDeletions");
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (channel) {
    try {
      ...
      console.log(
        `Channel ${channel.name} has been deleted by user interaction.`
      );
